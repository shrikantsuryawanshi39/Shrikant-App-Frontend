...
import { useUserClusterContext } from "../../context/UserClusterContext";

const UserCluster = () => {
    const { getUsers } = useUserContext();
    const { getAssignClusters, getUnAssignClusters, assignCluster, unAssignCluster } = useUserClusterContext();
    ...
    const fetchClusters = async (userId) => {
        ...
        try {
            const linkedClusters = await getAssignClusters(userId);
            const unlinkedClusters = await getUnAssignClusters(userId);
            setAssignedClusters(linkedClusters);
            setUnAssignedClusters(unlinkedClusters);
        } catch (error) {
    ...
    const assignUserToCluster = async (cluster) => {
        if (!selectedUserId) return alert("Please select a user first.");
        try {
            await assignCluster(selectedUserId, cluster.clusterId);
    ...
    const unassignUserFromCluster = async (cluster) => {
        if (!selectedUserId) return alert("Please select a user first.");
        try {
            await unAssignCluster(selectedUserId, cluster.clusterId);
            await fetchClusters(selectedUserId);
        } catch (error) {
                        ...
                        <ul className="px-4 overflow-y-auto max-h-[200px] scrollbar-hidden">
                            {filteredAssigned.map((cluster) => (
                                <li key={cluster.clusterId}
                                    onClick={() => unassignUserFromCluster(cluster)}
                                    className="cursor-pointer my-2 bg-gray-100 p-2 rounded hover:bg-gray-200 transition-all duration-200">
                                    <label>
                    ...
                    <div className="border rounded bg-white shadow w-xl max-h-[250px] overflow-y-auto">
                        ...
                        <ul className="px-4 overflow-y-auto max-h-[200px] scrollbar-hidden">
                            {filteredUnassigned.map((cluster) => (
                                <li key={cluster.clusterId}
                                    onClick={() => assignUserToCluster(cluster)}
                                    className="cursor-pointer my-2 bg-gray-100 p-2 rounded hover:bg-gray-200 transition-all duration-200">
                                    <label>
